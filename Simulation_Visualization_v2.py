...
class Simulation_Visualization:
    ...
    def plot_total_customers_with_movement(self):
        ...
        in_transit_class_data = np.array([
            ...
            # CSVで保存
        df = pd.DataFrame(index=[f"Node {node}" for node in range(self.N)])
        for cls in range(self.R):
            df[f"Class {cls} In System"] = in_system_class_data[cls]
            df[f"Class {cls} In Transit"] = in_transit_class_data[cls]
        
        output_csv = "log/class_based_total_customers_with_movement.csv"
        df.to_csv(output_csv)
        print(f"Class-based customer data saved to CSV: {output_csv}")
        ...
        bottoms = np.zeros(self.N)
        for cls in range(self.R):
            plt.bar(nodes, in_transit_class_data[cls], bottom=bottoms, label=f"Class {cls} In Transit")
            bottoms += in_transit_class_data[cls]

        # グラフの設定
        plt.xlabel("Node")
        plt.ylabel("Average Transit Customers")
        plt.title("Class-Based Average Transit Customers per Node")
        plt.xticks(nodes, [f"{node}" for node in range(self.N)])
        plt.legend(title="Customer Class", loc="upper right")
        plt.grid(axis="y", linestyle="--", alpha=0.7)

        # 画像保存
        output_transit_file = "log/class_based_transit_customers.png"
        plt.savefig(output_transit_file)
        plt.close()
        print(f"Class-based transit customer graph saved: {output_transit_file}")

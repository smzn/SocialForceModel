...
class BCMP_MVA:
    ...
    def __init__(self, N, R, K, mu, type_list, p, m):
        ...
        self.mu = np.array(mu)  # 1次元配列として受け取る
            ...
            for n in range(self.N): #P336 (8.43)
                for r in range(self.R):
                    if self.type_list[n] == 3:
                        self.T[n, r, k_state] = 1 / self.mu[n]
                        ...
                        if self.m[n] == 1: #P336 (8.43) Type-1,2,4 (m_i=1)
                            ...
                            self.T[n, r, k_state] = 1 / self.mu[n] * (1 + sum_l)
                        ...
                        if self.m[n] > 1:
                            ...
                            self.T[n, r, k_state] = 1 / (self.m[n] * self.mu[n]) * (1 + sum_l + sum_pi)
    ...
    def getPi(self, n, j, k, kr):
        ...
        if j == 0 and sum(kkr) > 0: #(8.45)
            ...
            for _r in range(self.R):
                sum_emlam += self.alpha[_r][n] / self.mu[n] * self.lmd[_r][state_number]
    ...
    def getPi8_44(self, n, j, k, state_number): #(8.45)から(8.44)を呼び出すときだけ利用 (人数を減らさず呼び出し)
        ...
        for _r in range(self.R):
            ...
            if kr_state_number < 0:
                ...
                sum_val += self.alpha[_r][n] / self.mu[n] * self.lmd[_r][state_number] * self.Pi[n][j-1][kr_state_number]
...
if __name__ == '__main__':
    ...
    p = pd.read_csv('transition_matrix.csv', header=None).values.tolist()

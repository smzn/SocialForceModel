...
class BCMP_Simulation:
    ...
    def calculate_rmse_summary(self):
        ...
        # --- クラス別の拠点ごとの平均系内人数を計算 ---
        final_time_idx = len(self.times) - 1  # 最終時刻のインデックス
        final_rmse_stats = []
        for node in range(self.N):
            row = {"node_id": node}
            for cls in range(self.R):
                row[f"Class{cls}"] = self.mean_total_class_data[node][cls][final_time_idx]
            final_rmse_stats.append(row)

        # --- CSVに保存 ---
        # 最終RMSE時のデータ
        final_csv_file = "./log/mean_L_with_transit_final_rmse.csv"
        pd.DataFrame(final_rmse_stats).to_csv(final_csv_file, index=False)
        print(f"Final RMSE average total customers (with transit) saved to {final_csv_file}")

        return final_total_rmse, final_class_rmse
    ...
    def calculate_rmse_without_transit(self):
        ...
        # --- クラス別の拠点ごとの平均系内人数を計算 ---        
        # 最終時刻を特定
        final_time_idx = len(self.times) - 1
        # 最終RMSE時のデータを計算
        final_rmse_stats = []
        for node in range(self.N):
            row = {"node_id": node}
            for cls in range(self.R):
                row[f"Class{cls}"] = self.mean_in_system_class_data[node][cls][final_time_idx]
            final_rmse_stats.append(row)
        # --- CSVに保存 ---
        # 最終RMSE時のデータ
        final_csv_file = "./log/mean_L_no_transit_final_rmse.csv"
        pd.DataFrame(final_rmse_stats).to_csv(final_csv_file, index=False)
        print(f"Final RMSE average in-system customers saved to {final_csv_file}")
